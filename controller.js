...
function getpoint(x,y) {
    ...
    var vs = [[x,y+47],[x+52,y-23.5],[x+52,y+23.5],[x,y-47],[x-52,y+23.5],[x-52,y-23.5]];
...
function isInside(x,y,vs){
    centerx=vs.x;
    centery=vs.y;
    var vs = [[centerx,centery+30],[centerx+30,centery-15],[centerx+30,centery+15],[centerx,centery-30],[centerx-30,centery+15],[centerx-30,centery-15]];
    var inside = false;
    for (var i = 0, j = vs.length - 1; i < vs.length; j = i++) {
        var xi = vs[i][0], yi = vs[i][1];
        var xj = vs[j][0], yj = vs[j][1];
